...
describe('QRCodeScanner', () => {
  ...
  it('renders scanner interface', () => {
    ...
    expect(screen.getByText('Наведите камеру на QR-код документа')).toBeInTheDocument()
  ...
  it('simulates QR code scan after delay', async () => {
    ...
    await waitFor(() => {
      expect(mockOnScan).toHaveBeenCalledWith('https://qr.pti.ru/r/3D-00001234/B/3?ts=1703123456&t=abc123def456')
    }, { timeout: 3000 })
  })
  ...
  it('renders with English language', () => {
    ...
    expect(screen.getByText('Point camera at document QR code')).toBeInTheDocument()
  ...
  it('shows scanning animation when scanning', () => {
    ...
    const scanningDots = screen.getAllByRole('generic').filter(el => 
  ...
  it('renders QR code overlay', () => {
    ...
    const overlay = screen.getByRole('generic').querySelector('.border-primary-500')
