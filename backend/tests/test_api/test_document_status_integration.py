...
class TestDocumentStatusIntegration:
    ...
    def test_workflow_without_authentication(self, client: TestClient):
        ...
        status_response = client.get("/api/v1/documents/TEST-DOC-001/revisions/A/status?page=1")
    ...
    def test_workflow_with_different_users(self, client: TestClient, test_user, test_admin_user):
        ...
        user_token = jwt.encode(token_data_user, settings.JWT_SECRET_KEY, algorithm=settings.JWT_ALGORITHM)
        ...
        admin_token = jwt.encode(token_data_admin, settings.JWT_SECRET_KEY, algorithm=settings.JWT_ALGORITHM)
        ...
        assert user_data["metadata"]["access_level"] == admin_data["metadata"]["access_level"]
    ...
    def test_workflow_with_nonexistent_document(self, client: TestClient, test_user):
        ...
        response_no_auth = client.get("/api/v1/documents/NONEXISTENT/revisions/A/status?page=1")
    ...
    def test_workflow_caching_behavior(self, client: TestClient, test_user):
        ...
        response1 = client.get("/api/v1/documents/TEST-DOC-001/revisions/A/status?page=1")
        assert response1.status_code == 200
        data1 = response1.json()
        assert data1["metadata"]["access_level"] == "limited"
        
        # Second request without authentication (should be cached)
        response2 = client.get("/api/v1/documents/TEST-DOC-001/revisions/A/status?page=1")
    ...
    def test_workflow_error_handling(self, client: TestClient):
        ...
        response = client.get("/api/v1/documents/TEST-DOC-001/revisions/A/status?page=0")
        ...
        response = client.get("/api/v1/documents/TEST-DOC-001/revisions/INVALID/status?page=1")
        assert response.status_code == 200  # Should still work, just with different revision
    ...
    def test_workflow_performance_with_auth(self, client: TestClient, test_user):
        ...
        start_time = time.time()
        response = client.get("/api/v1/documents/TEST-DOC-001/revisions/A/status?page=1")
