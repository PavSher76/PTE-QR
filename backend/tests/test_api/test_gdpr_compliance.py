...
class TestGDPRCompliance:
    ...
    def test_gdpr_data_minimization_principle(self, client: TestClient):
        ...
        essential_fields = ["doc_uid", "revision", "page", "is_actual", "business_status", "links", "metadata"]
        for field in essential_fields:
            assert field in data, f"Essential field {field} is missing"
        
        # Verify sensitive fields are not exposed
        sensitive_fields = ["enovia_state", "released_at", "superseded_by", "last_modified"]
        for field in sensitive_fields:
            assert field not in data, f"Sensitive field {field} should not be exposed without authentication"
    ...
    def test_gdpr_purpose_limitation_principle(self, client: TestClient):
        ...
        for field in personal_data_fields:
            assert field not in str(data), f"Personal data field {field} should not be included"
    ...
    def test_gdpr_storage_limitation_principle(self, client: TestClient):
        ...
        for field in persistent_identifiers:
            assert field not in str(data), f"Persistent identifier {field} should not be included"
    ...
    def test_gdpr_right_to_erasure_simulation(self, client: TestClient):
        ...
        personal_data_indicators = ["email", "phone", "name", "address", "ssn", "passport"]
        response_str = str(data).lower()
        for indicator in personal_data_indicators:
            assert indicator not in response_str, f"Personal data indicator {indicator} found in response"
    ...
    def test_gdpr_privacy_by_design(self, client: TestClient):
        ...
        assert data["metadata"]["access_level"] == "limited"  # Default to minimal access
    ...
    def test_gdpr_breach_notification_simulation(self, client: TestClient):
        ...
        for indicator in sensitive_data_indicators:
            assert indicator not in response_str, f"Sensitive data indicator {indicator} found in response"
