...
async def get_document_status(
    ...
    try:
        ...
        # Mock document status response (simplified for testing)
        response_data = {
            "doc_uid": doc_uid,
            "revision": rev,
            "page": page,
        ...
        metrics_service.record_api_request("GET", f"/documents/{doc_uid}/revisions/{rev}/status", 200, duration)
        ...
        duration = time.time() - start_time
        metrics_service.record_api_request("GET", f"/documents/{doc_uid}/revisions/{rev}/status", 404, duration)
        raise
    except Exception as e:
        duration = time.time() - start_time
        metrics_service.record_api_request("GET", f"/documents/{doc_uid}/revisions/{rev}/status", 500, duration)
