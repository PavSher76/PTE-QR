...
async def generate_qr_codes(
    ...
    try:
        ...
        qr_results = qr_service.generate_qr_codes(
            doc_uid=doc_uid, revision=revision, pages=pages, style=style, dpi=dpi
        )
        ...
        return {"doc_uid": doc_uid, "revision": revision, "mode": mode, "items": items}
        ...
        logger.error("Failed to generate QR codes", error=str(e), exc_info=True)
...
async def generate_pdf_with_qr_codes(
    ...
    try:
        ...
        logger.error(
            "Failed to generate PDF with QR codes", error=str(e), exc_info=True
        )
        raise HTTPException(status_code=500, detail="Internal server error")
