...
class ENOVIAClient:
    ...
    async def get_document_meta(self, doc_uid: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to get document metadata", doc_uid=doc_uid, error=str(e))
            return None
    ...
    async def get_revision_meta(self, doc_uid: str, revision: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            data = await self._make_request(
                "GET",
                f"/api/v1/documents/{doc_uid}/revisions/{revision}"
            )
    ...
    async def get_latest_released(self, doc_uid: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            data = await self._make_request(
                "GET",
                f"/api/v1/documents/{doc_uid}/revisions/latest"
            )
            ...
            logger.error("Failed to get latest released revision", doc_uid=doc_uid, error=str(e))
            return None
    ...
    def is_revision_actual(self, revision_data: Dict[str, Any]) -> bool:
        ...
        return (
            maturity_state in ["Released", "AFC", "Accepted", "Approved"] and
            not superseded_by
        )
