...
from app.models.document import EnoviaStateEnum, DocumentStatusEnum
...
class ENOVIAClient:
    ...
    async def _get_access_token(self) -> str:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                self._token_expires_at = datetime.now() + datetime.timedelta(seconds=expires_in - 60)
                
                logger.info("ENOVIA access token obtained")
                return self._access_token
                
        except Exception as e:
            logger.error("Failed to get ENOVIA access token", error=str(e), exc_info=True)
            raise
    ...
    def is_revision_actual(self, revision_meta: Dict[str, Any]) -> bool:
        ...
        return (
            not superseded_by and 
            enovia_state not in ["Obsolete", "Superseded"]
        )
